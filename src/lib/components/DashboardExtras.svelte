<script lang="ts">
  ...
  const weekDates = $derived.by(() => {
    ...
    return dayNames.map((name, index) => {
      ...
      return { name, key, reviews, seconds: activity[key]?.seconds ?? 0, today: key === dateKey(today) };
    });
  });
  ...
  let milestones = $derived.by(() => {
    ...
    return [
      { label: "Erste Runde", value: totalReviews, target: 10, suffix: "Wiederholungen" },
      { label: "Eigene Bibliothek", value: cards, target: 50, suffix: "Karten" },
      { label: "Im Rhythmus", value: streak, target: 7, suffix: "Tage in Folge" },
      { label: "Fest verankert", value: totalReviews, target: 100, suffix: "Wiederholungen" },
    ];
  });
  ...
  async function createQuickCard() {
    ...
    try {
      ...
      quickFront = "";
      quickBack = "";
      quickMessage = "Karte angelegt";
      onCardCreated();
    } catch {
      quickMessage = "Karte konnte nicht angelegt werden";
    } finally {
      quickSaving = false;
    }
  }
...
{#if moduleId === "continue"}
  <div class="surface-panel h-full p-5">
    <p class="section-kicker mb-3">Weiterlernen</p>
    {#if lastDeck}
      <h2 class="font-card-serif text-xl font-normal text-primary dark:text-primary-dark">{lastDeck.name}</h2>
      <button onclick={() => onStudyDeck(lastDeck!)} class="primary-action mt-4 px-4 py-2 text-sm">Runde fortsetzen</button>
    {:else}
      <p class="text-sm text-secondary">Sobald du einen Stapel angelegt hast, kannst du hier direkt weitermachen.</p>
    {/if}
  </div>
{:else if moduleId === "problems"}
  <div class="surface-panel h-full p-5">
    <div class="flex items-start justify-between gap-4">
      <p class="section-kicker mb-2">Problemkarten</p>
      <span class="font-pixel text-lg text-primary dark:text-primary-dark">{problemLoading ? "..." : problemCards.length}</span>
    </div>
    <button
      onclick={() => onStudyCards(problemCards, "Problemkarten")}
...
{:else if moduleId === "weekPlan"}
  <div class="surface-panel h-full p-5">
    <div class="flex items-start justify-between gap-4">
      <div>
        <p class="section-kicker mb-2">Wochenplan</p>
        <p class="text-sm text-secondary">{weekReviews} Wiederholungen in dieser Woche</p>
      </div>
      <button onclick={onStudyToday} disabled={!dashboard || dashboard.due_cards === 0} class="secondary-action px-3 py-1.5 text-xs disabled:opacity-40">Heute lernen</button>
    </div>
    <div class="mt-5 grid grid-cols-7 gap-2">
      {#each weekDates as day}
        <div class="flex min-w-0 flex-col items-center gap-2">
          <div class="flex h-16 w-full items-end overflow-hidden rounded bg-[#EEF1F5] dark:bg-[#171B23]">
            <div class="w-full bg-accent-easy dark:bg-accent-easy-dark" style="height: {Math.max(day.reviews > 0 ? 14 : 4, Math.min(100, day.reviews * 7))}%"></div>
          </div>
          <span class="text-[10px] font-semibold {day.today ? 'text-accent-correct' : 'text-secondary'}">{day.name}</span>
...
{:else if moduleId === "quickCapture"}
  <form class="surface-panel h-full p-5" onsubmit={(event) => { event.preventDefault(); void createQuickCard(); }}>
    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <p class="section-kicker mb-2">Schnellerfassung</p>
      <select bind:value={selectedDeckId} class="rounded-md border border-[#D8DEE8] bg-white px-3 py-2 text-sm text-primary dark:border-[#303744] dark:bg-[#151922] dark:text-primary-dark">
        {#each decks as deck}<option value={deck.id}>{deck.name}</option>{/each}
      </select>
    </div>
    <div class="mt-4 grid gap-3 sm:grid-cols-2">
      <input bind:value={quickFront} aria-label="Vorderseite" placeholder="Vorderseite" class="rounded-md border border-[#D8DEE8] bg-white px-3 py-2.5 text-sm text-primary outline-none focus:border-accent-correct dark:border-[#303744] dark:bg-[#151922] dark:text-primary-dark" />
      <input bind:value={quickBack} aria-label="Rückseite" placeholder="Rückseite" class="rounded-md border border-[#D8DEE8] bg-white px-3 py-2.5 text-sm text-primary outline-none focus:border-accent-correct dark:border-[#303744] dark:bg-[#151922] dark:text-primary-dark" />
    </div>
    <div class="mt-3 flex items-center justify-between gap-3">
      <span class="text-xs text-secondary" aria-live="polite">{quickMessage}</span>
      <button type="submit" disabled={quickSaving || !selectedDeckId || !quickFront.trim() || !quickBack.trim()} class="primary-action px-4 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-40">{quickSaving ? "Speichert..." : "Karte anlegen"}</button>
    </div>
  </form>
{:else if moduleId === "learningTime"}
  <div class="surface-panel h-full p-5">
    <p class="section-kicker mb-3">Lernzeit</p>
    <div class="grid grid-cols-2 gap-3">
      <div class="rounded-lg border border-[#E4E7EC] p-3 dark:border-[#2A303B]">
        <p class="text-xs text-secondary">Heute</p>
        <p class="font-pixel mt-2 text-sm text-primary dark:text-primary-dark">{formatDuration(todaySeconds)}</p>
      </div>
      <div class="rounded-lg border border-[#E4E7EC] p-3 dark:border-[#2A303B]">
        <p class="text-xs text-secondary">Diese Woche</p>
...
{:else if moduleId === "milestones"}
  <div class="surface-panel h-full p-5">
    <div class="flex items-start justify-between gap-4">
      <p class="section-kicker mb-2">Meilensteine</p>
      <span class="font-pixel text-lg text-accent-hard dark:text-accent-hard-dark">{milestones.filter((item) => item.value >= item.target).length}/{milestones.length}</span>
    </div>
      ...
      {#each milestones as item}
        ...
        <div class="rounded-lg border border-[#E4E7EC] p-3 dark:border-[#2A303B]">
          <div class="flex items-center justify-between gap-2">
            ...
            <span class="text-xs font-semibold {complete ? 'text-accent-easy dark:text-accent-easy-dark' : 'text-secondary'}">{complete ? "Erreicht" : `${Math.min(item.value, item.target)}/${item.target}`}</span>
          </div>
          <p class="mt-1 text-xs text-secondary">{item.suffix}</p>
          <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-[#E4E7EC] dark:bg-[#2A303B]">
            <div class="h-full rounded-full {complete ? 'bg-accent-easy dark:bg-accent-easy-dark' : 'bg-accent-hard dark:bg-accent-hard-dark'}" style="width: {Math.min(100, (item.value / item.target) * 100)}%"></div>
          </div>
        </div>
      {/each}
    </div>
...
{:else if moduleId === "timer"}
  <div class="surface-panel h-full p-5">
    <div class="flex items-start justify-between gap-4">
      <p class="section-kicker mb-2">Lerntimer</p>
      <span class="font-pixel text-2xl text-primary dark:text-primary-dark" aria-live="polite">{formatTimer(timerRemaining)}</span>
    </div>
    <div class="mt-4 h-2 overflow-hidden rounded-full bg-[#E4E7EC] dark:bg-[#2A303B]">
      <div class="h-full bg-accent-correct" style="width: {100 - (timerRemaining / (timerMinutes * 60)) * 100}%"></div>
    </div>
    <div class="mt-4 flex flex-wrap items-center justify-between gap-2">
      <div class="flex items-center gap-2">
        {#each [15, 25, 45] as minutes}
          <button onclick={() => selectTimer(minutes)} disabled={timerRunning} class="{timerMinutes === minutes ? 'primary-action' : 'secondary-action'} px-3 py-1.5 text-xs disabled:opacity-40">{minutes} Min.</button>
        {/each}
      </div>
      <div class="flex items-center gap-2">
        <button onclick={resetTimer} class="secondary-action px-3 py-1.5 text-xs">Zurücksetzen</button>
        <button onclick={toggleTimer} class="primary-action min-w-20 px-3 py-1.5 text-xs">{timerRunning ? "Pause" : timerRemaining === 0 ? "Neu starten" : "Start"}</button>
      </div>
    </div>
  </div>
{/if}
