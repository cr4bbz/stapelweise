...
#[tauri::command(rename_all = "camelCase")]
pub fn get_exam_template(state: State<DbState>, id: String) -> Result<Option<ExamTemplate>, CommandError> {
    let db = state.lock().map_err(|e| CommandError::from(format!("Lock error: {}", e)))?;
...
pub fn submit_exam_question_answer(
    ...
    let db = state.lock().map_err(|e| CommandError::from(format!("Lock error: {}", e)))?;
    let question = db.repo.submit_exam_question_answer(&question_id, &user_answer)?;
    Ok(question)
}
